...
from gis.models import Note, AnnotatedNote


class InvalidLazyUserResource(ModelResource):
    notes = ToManyField('gis.api.resources.FooResource', 'notes')
...
class LazyUserResource(ModelResource):
    notes = ToManyField('gis.tests.resources.NoteResource', 'notes')
...
class NoteModelResourceTestCase(TestCase):
    ...
    def test_lazy_relations(self):
        ...
        self.assertEqual(ilur.notes.to, 'gis.api.resources.FooResource')
        self.assertEqual(nplur.notes.to, 'FooResource')
        self.assertEqual(lur.notes.to, 'gis.tests.resources.NoteResource')
