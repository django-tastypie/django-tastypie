...
from accept_header_routing.models import Note, AnnotatedNote


class InvalidLazyUserResource(ModelResource):
    notes = ToManyField('accept_header_routing.api.resources.FooResource', 'notes')
...
class LazyUserResource(ModelResource):
    notes = ToManyField('accept_header_routing.tests.resources.NoteResource', 'notes')
...
class NoteModelResourceTestCase(TestCase):
    ...
    def test_lazy_relations(self):
        ...
        self.assertEqual(ilur.notes.to, 'accept_header_routing.api.resources.FooResource')
        self.assertEqual(nplur.notes.to, 'FooResource')
        self.assertEqual(lur.notes.to, 'accept_header_routing.tests.resources.NoteResource')
